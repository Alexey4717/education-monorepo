...
describe('/post', () => {
    ...
    it(`create 6 posts then: like post 1 by user 1, user 2; like post 2 by user 2, user 3; dislike post 3 by user 1; like post 4 by user 1, user 4, user 2, user 3; like post 5 by user 2, dislike by user 3; like post 6 by user 1, dislike by user 2. Get the posts by user 1 after all likes NewestLikes should be sorted in descending; `, async () => {
        ...
        expect(post1User1AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            2
        );
        expect(
            post1User1AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(0);
        expect(post1User1AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(post1User2AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(post1NotAuthAfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(
            post1NotAuthAfterReactions.body.extendedLikesInfo.likesCount
        ).toBe(2);
        expect(post1NotAuthAfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.None
        );
        ...
        expect(post2User2AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            2
        );
        expect(
            post2User2AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(0);
        expect(post2User2AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(post3User1AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            0
        );
        expect(
            post3User1AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(1);
        expect(post3User1AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Dislike
        );
        ...
        expect(post4User1AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            4
        );
        expect(
            post4User1AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(0);
        expect(post4User1AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(post5User2AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            1
        );
        expect(
            post5User2AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(1);
        expect(post5User2AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(post6User1AfterReactions.body.extendedLikesInfo.likesCount).toBe(
            1
        );
        expect(
            post6User1AfterReactions.body.extendedLikesInfo.dislikesCount
        ).toBe(1);
        expect(post6User1AfterReactions.body.extendedLikesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(
            post1User1AfterReactions.body.extendedLikesInfo.newestLikes.every(
                (
                    newestLike: NewestLikeType,
                    index: number,
                    array: NewestLikeType[]
                ) => {
                    if (index === 0) return true;
                    const currentLikeDate = new Date(
                        newestLike.addedAt
                    ).valueOf();
                    const prevLikeDate = new Date(
                        array[index - 1].addedAt
                    ).valueOf();
        ...
        expect(
            ...
            post1User1AfterReactions.body.extendedLikesInfo.newestLikes.sort(
    ...
    it(`create post then: like the post by user 1, user 2, user 3, user 4. get the post after each like by user 1. NewestLikes should be sorted in descending`, async () => {
        ...
        expect([
            {
                login: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[0].login,
                userId: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[0].userId,
            },
            {
                login: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[1].login,
                userId: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[1].userId,
            },
            {
                login: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[2].login,
                userId: post1ForUser1Reactions1.body.extendedLikesInfo
                    .newestLikes[2].userId,
            },
        ]).toEqual([
            {
        ...
        expect([
            {
                login: post1ForUser1Reactions2.body.extendedLikesInfo
                    .newestLikes[0].login,
                userId: post1ForUser1Reactions2.body.extendedLikesInfo
                    .newestLikes[0].userId,
            },
            {
                login: post1ForUser1Reactions2.body.extendedLikesInfo
    ...
    it(`shouldn't create post with incorrect input data`, async () => {
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription1,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription2,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription3,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription4,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription5,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
        ...
        await request(app)
            ...
            .set('Authorization', `Basic ${encodedBase64Token}`)
            .send({
                ...invalidInputData.shortDescription6,
                blogId: createdBlogId,
            })
            .expect(constants.HTTP_STATUS_BAD_REQUEST);
...
describe('comments in post', () => {
    ...
    it(`create 6 comments then: like comment 1 by user 1, user 2; like comment 2 by user 2, user 3; dislike comment 3 by user 1; like comment 4 by user 1, user 4, user 2, user 3; like comment 5 by user 2, dislike by user 3; like comment 6 by user 1, dislike by user 2`, async () => {
        ...
        expect(comment1User1AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment1User1AfterReactions.body.likesInfo.likesCount).toBe(2);
        expect(comment1User1AfterReactions.body.likesInfo.dislikesCount).toBe(
            0
        );
        expect(comment1User1AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(comment1NotAuthAfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment1NotAuthAfterReactions.body.likesInfo.likesCount).toBe(2);
        expect(comment1NotAuthAfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.None
        );
        ...
        expect(comment2User2AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment2User2AfterReactions.body.likesInfo.likesCount).toBe(2);
        expect(comment2User2AfterReactions.body.likesInfo.dislikesCount).toBe(
            0
        );
        expect(comment2User2AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(comment3User1AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment3User1AfterReactions.body.likesInfo.likesCount).toBe(0);
        expect(comment3User1AfterReactions.body.likesInfo.dislikesCount).toBe(
            1
        );
        expect(comment3User1AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Dislike
        );
        ...
        expect(comment4User1AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment4User1AfterReactions.body.likesInfo.likesCount).toBe(4);
        expect(comment4User1AfterReactions.body.likesInfo.dislikesCount).toBe(
            0
        );
        expect(comment4User1AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(comment5User2AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment5User2AfterReactions.body.likesInfo.likesCount).toBe(1);
        expect(comment5User2AfterReactions.body.likesInfo.dislikesCount).toBe(
            1
        );
        expect(comment5User2AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Like
        );
        ...
        expect(comment6User1AfterReactions.status).toBe(
            constants.HTTP_STATUS_OK
        );
        expect(comment6User1AfterReactions.body.likesInfo.likesCount).toBe(1);
        expect(comment6User1AfterReactions.body.likesInfo.dislikesCount).toBe(
            1
        );
        expect(comment6User1AfterReactions.body.likesInfo.myStatus).toBe(
            LikeStatus.Like
        );
    ...
    it(`should return 201 and the newly created comment in post if correct input data`, async () => {
        ...
        const { id: commentId, createdAt } = createdComment3.body;

        expect(createdComment3.body.content).toBe(
