...
import {
    GetBlogsInputModel,
    SortBlogsBy,
} from '../models/BlogModels/GetBlogsInputModel';
...
export const blogControllers = {
    async getBlogs(
        ...
        const resData = await blogsQueryRepository.getBlogs({
            ...
            sortBy: (req.query.sortBy?.toString() ||
                'createdAt') as SortBlogsBy, // by-default createdAt
    ...
    async getBlog(
        ...
        res.status(constants.HTTP_STATUS_OK).json(
            getMappedBlogViewModel(resData)
        );
    },
    ...
    async createBlog(
        ...
        res.status(constants.HTTP_STATUS_CREATED).json(
            getMappedBlogViewModel(createdBlog)
        );
    },
