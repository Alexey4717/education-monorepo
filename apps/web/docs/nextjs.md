...
Создали константу export const NO_INDEX_PAGE = { robots: { index: false, follow: false } }; для метаданных
Чтобы на некоторых страницах отключить индексацию, чтоб поисковик не пытался туда сувать любые (неактуальные,
нетематичные) значения.

При поиске через queryParams, их надо помещать в массив queryKey хуков tanstack, чтоб обновлялись ключи кеширования.

Чтобы редиректить программно, нужно использовать router.push:

```tsx
import { useRouter } from 'next/navigation'; // не next/router

const router = useRouter(); // лучше не деструктуризировать, т.к. иногда теряется контекст и не работает
