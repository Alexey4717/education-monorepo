...
export function useVideoPlayer({ fileName, toggleTheaterMode }: Props) {
	...
	const { isPlaying, togglePlayPause, setIsPlaying } = usePlayPause(
		playerRef,
		bgRef
	);
	const { currentTime, progress, videoTime, setCurrentTime } =
		useVideoProgress(playerRef);
	...
	const { changeVolume, isMuted, toggleMute, volume } =
		useVideoVolume(playerRef);
