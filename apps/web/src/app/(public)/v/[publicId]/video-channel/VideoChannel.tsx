...
const DynamicSubscribeButton = dynamicNext(
	() =>
		import('@/components/SubscribeButton').then(
			(mod) => mod.SubscribeButton
		),
	{
		ssr: false,
	...
	return (
		<div className="flex items-center justify-between mb-6">
			<div className="flex gap-4 items-center">
				...
				<div>
					<Link href={PAGE.CHANNEL(video.channel.slug)}>
						<Heading className="mb-0" classNameHeading="text-lg">
					...
					<div className="text-gray-400 text-sm flex items-center gap-1">
						{transformCount(video.channel.subscribers.length)}{' '}
						subscribers
					</div>
				</div>
			</div>
